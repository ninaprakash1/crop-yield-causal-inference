...
set.seed(42)
p_values_log_ratio = vector(length = length(crop_types))
p_values_diff_mean = vector(length = length(crop_types))
n_iters = 10000

for (i in 1:length(crop_types)) {
  # cat('\nRunning FRT for ',crop_types[i], '...')
  crop.msk = data$Crop == crop_types[i]
  p_val_lr = compute_p_value_mc(f = generate_diff_test_stat,
                             diff_vec = data$log.Yield.Ratio[crop.msk],
                             n_draw = 10000,
                             seed = 100)
  p_values_log_ratio[i] = p_val_lr
  # cat('\n\tp_value for log ratio: ', p_val_lr)
  
  p_val_dm = compute_p_value_mc(f = generate_diff_test_stat,
                             diff_vec = data$Yield.Diff[crop.msk],
                             n_draw = 10000,
                             seed = 100)
  p_values_diff_mean[i] = p_val_dm
  # cat('\n\tp_value for diff means: ', p_val_dm)
}

FRT_results = data.frame(crop_types, p_values_log_ratio, p_values_diff_mean)
colnames(FRT_results) = c("crop", "p_val_log_ratio", "p_val_diff_mean")
(FRT_results)
...
# Generate vectors of t-test results in the same order as crop_types
...
CLT_results = data.frame(crop_types, diff_in_yield_tau, diff_in_yield_p_vals, diff_in_yield_CI_lower, diff_in_yield_CI_upper)
colnames(CLT_results) = c("crop","tau_CLT", "p_val", "CI_lower","CI_upper")
(CLT_results)
